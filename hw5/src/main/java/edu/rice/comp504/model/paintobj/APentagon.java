...
public class APentagon extends APaintObject {
    /*Add new attributes to the concrete shape "BALL"*/
    private int side;

    /**
     * Constructor of APentagon
     * */
    private APentagon(Point loc, Point vel, String color, IUpdateStrategy strategy, int side){
        super(loc, vel, color, "Pentagon", strategy);
        this.side = side;
    }
     ...
     * */
    public static APentagon makePentagon(IUpdateStrategy strategy, Point dims){
        ...
        // Return a new object
        return new APentagon(loc, vel, "#EEEE0A", strategy, side);
    }
    ...
    public void collision(Point dims){
        ...
        if(this.getLocation().x + this.side > dims.x){
            ...
            Point newVel = new Point(-vel.x, vel.y);
